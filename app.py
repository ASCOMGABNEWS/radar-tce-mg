...
/* Nunca quebrar o texto dos botões de abrangência */
.st-key-abrangencia-total button p,
.st-key-abrangencia-estadual button p,
.st-key-abrangencia-nacional button p,
...
/* Estado selecionado: preto e sem quebra de linha */
...
.st-key-abrangencia-total-selecionada button p,
.st-key-abrangencia-estadual-selecionada button p,
.st-key-abrangencia-nacional-selecionada button p {
    white-space: nowrap !important;
}
.st-key-abrangencia-total button *,
.st-key-abrangencia-estadual button *,
...
# ============================================================
# MONITORAMENTO DE MÍDIAS / JORNAIS
# ============================================================
...
if _ticker_final:
    ...
    components.html(
        ...
        <div class="radar-ticker-wrap">
            <div class="radar-ticker-label">📰 ÚLTIMAS NOTÍCIAS DOS PORTAIS DE MG</div>
...
with col_total:
    with st.container(key="abrangencia-total-selecionada" if st.session_state.get("abrangencia_botao", "Todas") == "Todas" else "abrangencia-total"):
        if st.button(
            "Abrangência Total",
...
with col_estadual:
    with st.container(key="abrangencia-estadual-selecionada" if st.session_state.get("abrangencia_botao", "Todas") == "Minas Gerais" else "abrangencia-estadual"):
        if st.button(
...
with col_nacional:
    with st.container(key="abrangencia-nacional-selecionada" if st.session_state.get("abrangencia_botao", "Todas") == "Nacional" else "abrangencia-nacional"):
        if st.button(
